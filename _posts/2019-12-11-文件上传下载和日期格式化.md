...
layout:     post
title:      文件上传下载和日期格式化
subtitle:   在开发过程中会经常遇到文件上传下载和需要日期格式化的地方~
date:       2019-12-11
author:     guohang
header-img: img/post-bg-fileupload.jpg
catalog: true
tags:
    - SpringMVC
    - 文件上传下载
...
Spring MVC通过MultipartResolver实现⽂件上传功能。MultipartResolver是⼀个接⼝对象，需要通过它的实现类CommonsMultipartResolver来完成⽂件上传⼯作。
